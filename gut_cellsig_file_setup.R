...
library(ggplot2)
library(reshape2)
#########gut ref with SCPs as a negative ctrl##############
...
scps=subset(adr_all, Annotation%in%c("SCPs"))
#get SCP matrix
...
scp_mtx=scps@assays$RNA@counts
rownames(scp_mtx)=ensembl_adrenal$V1
#get gut matrix
ensembl_gut=read.csv('onecs/obs.csv')
gut_rel_mtx=gut_srat@assays$RNA@counts
rownames(gut_rel_mtx)=ensembl_gut$gene_ids

comm_genes=intersect(rownames(scp_mtx),rownames(gut_rel_mtx))
#merge the matrices, change colnames to include annotation and write the files for cell signal analysis
merged_mtx=cbind(gut_rel_mtx[comm_genes,], scp_mtx[comm_genes,])
colnames(merged_mtx)=c(paste0(gut_srat@meta.data$final_annot,":",colnames(gut_rel_mtx)),
                       paste0("SCPs:", colnames(scp_mtx)))

writeMM(merged_mtx, "/lustre/scratch117/casm/team274/gk14/Dediff/cellsig_gut/sc_gut_with_scps/gut.mtx")
write.table(rownames(merged_mtx), "/lustre/scratch117/casm/team274/gk14/Dediff/cellsig_gut/sc_gut_with_scps/gut_rowNames.tsv", sep = "\t", quote = F, col.names = F, row.names = F)
write.table(colnames(merged_mtx), "/lustre/scratch117/casm/team274/gk14/Dediff/cellsig_gut/sc_gut_with_scps/gut_columnNames.tsv", sep = "\t", quote = F, col.names = F, row.names = F)

...
colon_mtx=readMM('matrix.mtx')

obs=read.csv("onecs/obs.csv")
cells=read.csv("onecs/var.csv")
rownames(cells)=cells$index
rownames(colon_mtx)=as.character(obs$index)
colnames(colon_mtx)=as.character(cells$index)

colon_seurat=CreateSeuratObject(colon_mtx, meta.data = cells)
...
colon_seurat=process_10x(colon_seurat)
saveRDS(colon_seurat, "/lustre/scratch117/casm/team274/gk14/human_colon.rds")
...
rownames(len)=make.unique(len$geneName)
len=len[which(rownames(len)%in%comm_ensg),]
gene_lengths=len$geneLengths


DimPlot(colon_seurat, group.by = "annotation_rasa") +theme()



colon_canc=read.csv("/home/jovyan/Dediff/GSE81861_CRC_tumor_all_cells_COUNT.csv")
...
keep_cells=colnames(colon_seurat)[which(!colon_seurat@meta.data$cell_group%in%c("doublet", "empty"))]

colon_seurat=subset(colon_seurat, cells=keep_cells)

colon_seurat@meta.data$cell_group_age=paste0(colon_seurat$age_cat, "_", colon_seurat@meta.data$cell_group)
...
train_for_bulk=trainModel(sc_mtx[comm_ensg,], colon_seurat@meta.data$exp_annot, workers=NULL)
...
ann_ord=c("fetal_Enterocyte","fetal_Goblet cell","fetal_crypt",
          ...
          "adult_Tuft cell","adult_BEST1 enterocyte","adult_Enteroendocrine","adult_Paneth","adult_Mesothelium", 
...
similarityHeatmap(col_exp_canc_pred, column_order=ann_ord)
similarityHeatmap(predict_bulk, column_order=ann_ord, row_split=mDat_col$gdc_cases.samples.sample_type)
